...
void Utility::ScreenPosToWorldRay(
	...
	out_direction = Vector3f(lRayDir_world).normalized();
}
...
void atRenderEngine::Utility::convertToRGBA(char *& dest, char *& src, int size)
{
	dest = new char[size * 4];
	for (int i = 0; i <size; i++)
	{
		dest[i * 4 + 0] = src[i];
		dest[i * 4 + 1] = src[i];
		dest[i * 4 + 2] = src[i];
		dest[i * 4 + 3] = (src[i]<30) ? 0 : 255;
	}
}
