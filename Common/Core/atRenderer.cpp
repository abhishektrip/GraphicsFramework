...
void atRenderer::atKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	// Move forward
	if (key == GLFW_KEY_UP && mods == 0 && action == GLFW_PRESS) 
	{		
		mCamera->HandleKeyPress(Utility::UP , aspectRatio);
	}
	// Move backward
	if (key == GLFW_KEY_DOWN && mods == 0 && action == GLFW_PRESS) 
	{
	...
	if (key == GLFW_KEY_UP && mods == GLFW_MOD_ALT && action == GLFW_PRESS)
	{
		mCamera->HandleKeyPress(Utility::PAN_UP, aspectRatio);
	}

	if (key == GLFW_KEY_DOWN && mods == GLFW_MOD_ALT && action == GLFW_PRESS)
	{
		mCamera->HandleKeyPress(Utility::PAN_DOWN, aspectRatio);
	}
	...
	if (key == GLFW_KEY_S && mods == GLFW_MOD_CONTROL && action == GLFW_PRESS)
	{
		...
		//{
		//	Utility::CorrespondenceMarkerData test;
		//	test.vertIdxSource = i;
		//	test.vertIdxTarget = i;

		//	markerData.push_back(test);
		//}
		...
		//	markerData.push_back(test);
		//}
		writer.SaveCorrespondenceFile("../data/Male_Nekei.corres", markerData);
	}		
}
	...
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		...
		if (meshIdxForSelection == 0)
		{
			if (mScenes[meshIdxForSelection]->GetIntersection(currentRay, mPreviousSelectedVertexInfo))
			{
				...
				//markerPair.vertIdxTarget = mPreviousSelectedVertexInfo.vertexListIdx;
